...
public class IntervalUtils {
		...
		while (matcher.find()) {
			...
			switch (matcher.group(3).toLowerCase().charAt(0)) {
				...
				case 's':
					...
					break;
				case 'm':
					time = time + (Long.parseLong(interval) * 20) * 60;
					break;
				case 'h':
					time = time + (Long.parseLong(interval) * 20) * 60 * 60;
					break;
				case 'd':
					time = time + (Long.parseLong(interval) * 20) * 60 * 60 * 24;
					break;
				case 'w':
					time = time + (Long.parseLong(interval) * 20) * 60 * 60 * 24 * 7;
					break;
				default:
					break;
			}
	...
	public static long getMilliseconds(String interval) {
		return getSeconds(interval) * 1000;
	}

	public static long getSeconds(String interval) {
